...
class HabitacionData {
	...
	public function add(){
		$sql = "insert into habitacion (nombre,descripcion,precio,id_categoria,estado,id_nivel,fecha_creada) ";
		$sql .= "value (\"$this->nombre\",\"$this->descripcion\",\"$this->precio\",$this->id_categoria,\"$this->estado\",$this->id_nivel,$this->fecha_creada)";
		Executor::doit($sql);
	}
...
// partiendo de que ya tenemos creado un objecto HabitacionData previamente utilizamos el contexto
	...
	public function updateRoom(){
		$sql = "update ".self::$tablename." set nombre=\"$this->nombre\",descripcion=\"$this->descripcion\",precio=\"$this->precio\",id_categoria=$this->id_categoria,id_nivel=$this->id_nivel where id=$this->id";
		Executor::doit($sql);
	}
