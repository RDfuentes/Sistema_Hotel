...
// partiendo de que ya tenemos creado un objecto CategoryData previamente utilizamos el contexto
	...
	public static function getEgresoDiario($hoy){
		$sql = "select * from ".self::$tablename." where  date(fecha)=\"$hoy\"  ";
		$query = Executor::doit($sql);
		return Model::many($query[0],new GastoData());
	}
	...
	public static function getLike($p){
		$sql = "select * from ".self::$tablename." where descripcion like '%$p%' ";
		$query = Executor::doit($sql);
		return Model::many($query[0],new GastoData());
	}
 ...
 $sql = "select * from ".self::$tablename." where day(fecha_creacion) = \"$dia\" and month(fecha_creacion) = \"$fecha\"   order by Fecha_creacion desc";
		$query = Executor::doit($sql);
		return Model::many($query[0],new GastoData());
	}
